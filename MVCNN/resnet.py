...
class BasicBlock(nn.Module):
    ...
    def __init__(
        self, inplanes, planes, stride=1, downsample=None, use_triplet_attention=False, att_name = "TripletAttention"
    ):
        ...
        print("basic block use_triplet_attention = ", use_triplet_attention)
        print("basic block attname = ", att_name)
        if use_triplet_attention:
            if att_name == "TripletAttention":
                self.triplet_attention = TripletAttention(planes, 16)
            elif att_name == "SENet":
                self.triplet_attention = SENet(planes, 16)
            elif att_name == "CBAM":
                self.triplet_attention = CBAM(planes, 16)
            elif att_name == "BAM":
                self.triplet_attention = BAM(planes)
    ...
    def __init__(
        ...
        if use_triplet_attention:
            if att_name == "TripletAttention":
                ...
                self.triplet_attention = SENet(planes * 4, 16)
            elif att_name == "CBAM":
                self.triplet_attention = CBAM(planes * 4, 16)
            elif att_name == "BAM":
                self.triplet_attention = BAM(planes * 4)
        ...
        if triplet_pos == 0 or triplet_pos == 2:
            layers.append(
                block(
                    ...
                    stride,
                    downsample,
                    use_triplet_attention= not(att_type == None),
                    att_name = att_type
                )
            )
