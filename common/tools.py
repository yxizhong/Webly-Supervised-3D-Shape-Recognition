...
def train(model, train_loader, optimizer, ceriation, epoch, device):
...
def evaluate(model, eva_loader, ceriation, prefix, device, ignore=-1):
    ...
    if prefix != "":
        ...
        print("[Test] ", prefix, round(top1.avg.item(), 2))

    return losses.avg, top1.avg.to("cpu", torch.float).item()


def evaluateWithBoth(model1, model2, eva_loader, prefix, device):
    top1 = AverageMeter('Acc@1', ':3.2f')
    model1.eval()
    model2.eval()
